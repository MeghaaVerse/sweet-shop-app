...
const NotificationPanel: React.FC = () => {
  ...
  const togglePanel = () => {
    if (bellRef.current) {
      ...
      setPanelPosition({
        top: rect.bottom + window.scrollY, // panel below the bell
        left: rect.right - 384, // align right edge (w-96 = 384px)
      });
    }
    setIsOpen(!isOpen);
  };
    ...
    <div className="relative">
      ...
      {isOpen && (
        <>
          ...
          {/* Panel */}
          <div
            className="absolute w-96 max-w-sm bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-gray-200 z-[9999] max-h-96 overflow-hidden"
            ...
            <div className="p-4 border-b border-gray-100">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-gray-900">
                  Notifications
                  {unreadCount > 0 && (
                    <span className="ml-2 text-sm text-pink-600 font-medium">({unreadCount} new)</span>
                  )}
                </h3>
