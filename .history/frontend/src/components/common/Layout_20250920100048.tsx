...
import { Candy, Package, BarChart3, LogOut, Plus } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // Initialize real notifications on mount
  useEffect(() => {
    const notificationService = NotificationService.getInstance();
    notificationService.initialize(addNotification);

    // Add a welcome notification on page load
    addNotification({
      title: '👋 Welcome back!',
      message: `Hello ${user?.name || 'Guest'}, ready to manage your sweet inventory?`,
      type: 'success',
    });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
           ...
           <div className="flex items-center space-x-4">
              {/* 🔔 Real Notification Panel replaces static bell */}
              <NotificationPanel />
      ...
      <button 
        ...
        className="fixed bottom-8 right-8 w-14 h-14 bg-gradient-to-r from-pink-500 to-purple-500 text-white rounded-full shadow-lg hover:shadow-xl transform hover:scale-110 transition-all duration-200 flex items-center justify-center group z-50"
        title="Quick Actions"
        >
