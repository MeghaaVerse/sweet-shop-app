...
const Sweets: React.FC = () => {
  ...
  if (!newSweet.name?.trim() || !newSweet.category?.trim()) {
    addNotification({
  ...
  if (newSweet.price === undefined || newSweet.price <= 0) {
    addNotification({
      title: 'Validation Error',
  ...
  if (newSweet.stock !== undefined && (!Number.isInteger(newSweet.stock) || newSweet.stock < 0)) {
    addNotification({
      title: 'Validation Error',
    ...
    try {
      const sweet = await sweetsAPI.create(newSweet);
  ...
  const handleEditSweet = (sweet: Sweet) => {
    addNotification({
      title: 'Edit Feature',
      message: `Opening edit form for ${sweet.name}`,
      type: 'info',
    });
  };
