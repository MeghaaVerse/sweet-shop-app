...
import type { Sweet, InventoryReport, StockAlert } from '../types';
...
const Dashboard: React.FC = () => {
  ...
  const [newSweetPrice, setNewSweetPrice] = useState<number | ''>('');
  ...
  const handleSweetSubmit = async() => {
    if (!newSweetName.trim()) {
      addNotification({
        type: 'error',
        title: 'Missing Information',
        message: 'Please enter a sweet name',
      });
      return;
    }

    if (!newSweetPrice || newSweetPrice <= 0) {
      addNotification({
        type: 'error',
        title: 'Invalid Price',
    ...
    try {
     console.log('Submitting sweet:', {
      name: newSweetName.trim(),
      description: '',
      price: Number(newSweetPrice),
      category: newSweetCategory.trim(),
      imageUrl: '',
      stock: 0,
    });

    
    // Actually call the API to create the sweet
    ...
    console.log('Sweet created:', newSweet);

    // Add the new sweet to the local state
    setSweets(prevSweets => [newSweet, ...prevSweets]);

    // Trigger success notification
    notificationService.onSweetAdded(newSweetName);
    ...
    // Trigger inventory notification after 2 seconds
    setTimeout(() => {
      notificationService.onInventoryUpdated(newSweetName, 'RESTOCK', 50);
    }, 2000);
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button 
              ...
              className="btn-primary flex items-center space-x-2 hover:scale-105 transform transition-all duration-200"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredSweets.map((sweet) => (
              <SweetCard
                key={sweet.id}
                sweet={sweet}
                onEdit={(sweet) => addNotification({
                  title: 'Edit Feature',
                  message: `Edit functionality for ${sweet.name} coming soon!`,
                  type: 'info'
                })}
                onDelete={(sweet) => addNotification({
                  title: 'Delete Feature',
                  message: `Delete functionality for ${sweet.name} coming soon!`,
                  type: 'info'
                })}
                onStockUpdate={(sweet) => addNotification({
                  title: 'Stock Update',
                  message: `Stock update for ${sweet.name} - redirecting to inventory...`,
                  type: 'info'
                })}
              />
            ))}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold text-gray-900">Quick Add Sweet</h3>
              <button
            ...
            <div className="space-y-4">
              ...
              <input
                ...
                placeholder="Price"
                value={newSweetPrice}
                onChange={(e) => setNewSweetPrice(e.target.value ? Number(e.target.value) : '')}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
              />
              <input
                type="text"
