...
const Sweets: React.FC = () => {
  ...
  const handleAddSweet = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSweet) {
        const updated = await sweetsAPI.update(editingSweet.id, payload);
        setSweets(prev => prev.map(s => (s.id === editingSweet.id ? updated : s)));
        notificationService.onSweetAdded(`Updated ${updated.name}`);
      } else {
        const sweet = await sweetsAPI.create(payload);
        setSweets(prev => [sweet, ...prev]);
        notificationService.onSweetAdded(sweet.name);
      }
      handleCloseModal();
    } catch (error: any) {
      addNotification({
 ...
 const handleStockUpdate = (sweet: Sweet) => {
  // Create a simple stock update modal
  const newStock = prompt(`Current stock for ${sweet.name}: ${sweet.stock}\nEnter new stock quantity:`, sweet.stock.toString());
  
  if (newStock !== null && !isNaN(Number(newStock))) {
    const stockNumber = Number(newStock);
    const difference = stockNumber - sweet.stock;
    
    // Update the sweet in local state
    setSweets(prevSweets => 
      prevSweets.map(s => 
        s.id === sweet.id ? { ...s, stock: stockNumber } : s
      )
    );
    ...
    addNotification({
      title: 'Stock Updated',
      message: `${sweet.name} stock updated to ${stockNumber} units (${difference > 0 ? '+' : ''}${difference})`,
      type: 'success',
    });
